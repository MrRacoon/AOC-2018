...
type num = int
const INPUT_FILE = "src/day1/input"

var sum: num = 0
var currentFreq: Option[num] = none(num)
var frqs: HashSet[num] = initSet[num]()

while isNone(currentFreq):
  echo "seenFreqs: ", len(frqs)
  for line in lines INPUT_FILE:
    var x = parseInt line
    sum = sum + x
    if contains(frqs, sum) and isNone(currentFreq):
      currentFreq = some(sum)
    incl(frqs, sum)
